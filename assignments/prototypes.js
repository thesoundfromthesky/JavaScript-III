...
// that inherit from the Humanoid constructor function.
// * Give the Hero and Villains different methods that could
// be used to remove health points from objects which could result in destruction
...
Hero.prototype.heroicAttack = function(target) {
  if (isNaN(target.damage)) this.damage = 0;
...
Villain.prototype.villainAttack = function(target) {
  if (isNaN(target.damage)) this.damage = 0;
  target.healthPoints -= this.damage;
  if (target.healthPoints < 0) {
    console.log(target.destroy());
...
function battle() {
  while (true) {
    if (hero.healthPoints < 0) break;
    hero.heroicAttack(villain);
    if (villain.healthPoints < 0) break;
